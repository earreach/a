...
class mall {
    ...
    function __construct($moduleid) {
		...
		$this->fields = array('catid','mycatid','areaid','level','title','style','subtitle','subtext','sublink','fee','introduce','brand','skuid','price','fprice','sprice','sfromtime','stotime','step','stock','minamount','amount','maxamount','days','unit','thumb','thumb1','thumb2','thumbs','video','tag','status','hits','username','editor','addtime','adddate','edittime','editdate','ip','template','linkurl','filepath','elite','note','company','truename','telephone','mobile','address','email','qq','wx','ali','skype','n1','n2','n3','v1','v2','v3','prices','express_1','express_name_1','fee_start_1','fee_step_1','express_2','express_name_2','fee_start_2','fee_step_2','express_3','express_name_3','fee_start_3','fee_step_3','cod');
    }
	...
	function set($post) {
		...
		if($post['mode'] == 3) {
			...
			$post['step'] = $post['stock'] = array();
			$post['prices'] = '';
			$post['fprice'] = dround($post['fprice'], 2, 1);
			if($post['fprice'] >= $post['price']) $post['fprice'] = 0.00;
			$post['sprice'] = dround($post['sprice'], 2, 1);
			if($post['sprice'] >= $post['price']) $post['sprice'] = 0.00;
			if($post['sprice'] > 0) {
				$sfromtime = is_time($post['sfromtime']) ? datetotime($post['sfromtime']) : 0;
				$stotime = is_time($post['stotime']) ? datetotime($post['stotime']) : 0;
				if($sfromtime && $stotime && $sfromtime < $stotime && $stotime > DT_TIME) {
					$post['sfromtime'] = $sfromtime;
					$post['stotime'] = $stotime;
				} else {
					$post['sprice'] = $post['sfromtime'] = $post['stotime'] = 0;
				}
			} else {
				$post['sprice'] = $post['sfromtime'] = $post['stotime'] = 0;
			}
		}
		if($post['mode'] > 0) $post['fprice'] = $post['sprice'] = $post['sfromtime'] = $post['stotime'] = 0;
		$post['minamount'] = intval($post['minamount']);
		$post['amount'] = intval($post['amount']);
		$post['maxamount'] = intval($post['maxamount']);
		$post['days'] = dround($post['days']);
	...
	function update($itemid) {
		...
		if($member && $member['shop']) $member['company'] = $member['shop'];
		if($member) $update .= update_user($member, $item);
		if($MOD['edit_areaid']) $update = cutstr($update, '', ',areaid');
	...
	function relate_add($M, $A, $N) {
		...
		DB::query("UPDATE {$this->table} SET relate_title='$title' WHERE itemid=$A[itemid]");
	}
	...
	function sublink($url) {
		if(!is_uri($url)) return '';
		if(strpos($url, 'redirect'.DT_EXT) !== false) return '';
		return $url;
	}
