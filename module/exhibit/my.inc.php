...
switch($action) {
	...
	case 'sign':
		...
		$fromtime = is_date($fromdate) ? datetotime($fromdate.' 00:00:00') : 0;
		...
		while($r = $db->fetch_array($result)) {
			$r['linkurl'] = gourl('?mid='.$moduleid.'&itemid='.$r['id']);
	...
	case 'show':
		...
		if(!$item || $item['user'] != $_username) message();
		$item['linkurl'] = gourl('?mid='.$moduleid.'&itemid='.$item['id']);
	break;
	case 'del':
		$itemid or message();
		$itemids = is_array($itemid) ? implode(',', $itemid) : $itemid;
		$db->query("DELETE FROM {$table_sign} WHERE itemid IN ($itemids) AND user='$_username'");
		dmsg($L['success_delete'], $forward);
	break;
	default:
...
if($DT_PC) {
	...
	$foot = '';
	if($action == 'add' || $action == 'edit' || $action == 'sign' || $action == 'show') {
		//
	} else {
