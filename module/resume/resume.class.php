		...
		if(is_array($itemid)) {
			...
			if($all) {
				...
				if($r['username'] && $MOD['credit_del']) {
					...
					credit_record($r['username'], -$MOD['credit_del'], 'system', lang('my->credit_record_del'), 'ID:'.$this->itemid);
				}
	...
	function check($itemid) {
		...
		if(is_array($itemid)) {
			...
			if($MOD['credit_add'] && $item['username'] && $item['hits'] < 1) {
				...
				credit_record($item['username'], $MOD['credit_add'], 'system', lang('my->credit_record_add'), 'ID:'.$this->itemid);
			}
