...
class price {
	...
	function get_list($condition = '1', $order = 'addtime DESC') {
		global $pages, $page, $offset, $pagesize, $MOD, $items, $sum;
		if($page > 1 && $sum) {
			$items = $sum;
		} else {
		...
		while($r = DB::fetch_array($result)) {
			...
			$r['linkurl'] = $MOD['linkurl'].rewrite('price'.DT_EXT.'?itemid='.$r['pid']);
			$pids[$r['pid']] = $r['pid'];
			$lists[] = $r;
		}
		if($pids) {
	...
	function update($itemid, $item = array()) {
		...
		if($item['username']) {
			...
			if($m) $sql = "company='$m[company]',telephone='$m[telephone]',qq='$m[qq]',wx='$m[wx]'";
		}
