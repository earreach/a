...
if($mid > 4 && isset($MODULE[$mid]) && !$MODULE[$mid]['islink']) {
	...
	if($MOD['feed_enable']) {
		...
		$condition = "status=3";
		if($MOD['feed_level']) $condition .= " AND level>0";
	...
	$split = is_file(DT_CACHE.'/'.$mid.'.part');
	...
	if($MOD['feed_enable']) {
		$result = $db->query("SELECT itemid,title,thumb,introduce,linkurl,addtime FROM {$table} WHERE {$condition} ORDER BY addtime DESC LIMIT 0,$pagesize", 'CACHE');
		while($r = $db->fetch_array($result)) {
			echo '<item id="'.$r['itemid'].'">';
			echo '<title><![CDATA['.$r['title'].']]></title>';
			echo '<link>'.$modurl.str_replace('&', '&amp;', $r['linkurl']).'</link>';
			echo '<thumb>'.str_replace('.thumb.', '.middle.', $r['thumb']).'</thumb>';
			echo '<description><![CDATA['.$r['introduce'].']]></description>';
			if($MOD['feed_content']) {
				$itemid = $r['itemid'];
				$tb = content_table($mid, $itemid, $split);
				$t = $db->get_one("SELECT content FROM {$tb} WHERE itemid=$itemid", 'CACHE');
				echo '<content><![CDATA['.$t['content'].']]></content>';
			}
