...
class gift {
	...
	function delete_order($itemid) {
		...
		$result = DB::query("SELECT * FROM {$this->table_order} WHERE oid IN ($itemids)");
		while($r = DB::fetch_array($result)) {
			DB::query("UPDATE {$this->table} SET orders=orders-1 WHERE itemid=$r[itemid]");
		}
		DB::query("DELETE FROM {$this->table_order} WHERE oid IN ($itemids)");
	}
