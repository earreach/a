...
class comment {
	...
	function set($post) {
		...
		if($post['reply']) {
			$post['replytime'] = is_time($post['replytime']) ? strtotime($post['replytime']) : DT_TIME;
		} else {
			$post['replytime'] = 0;
			$post['replyer'] = '';
		}
	...
	function clear($condition = 'status=0') {
		DB::query("DELETE FROM {$this->table} WHERE {$condition}");
	}
	...
	function get_ban_list($condition = '1') {
		...
		while($r = DB::fetch_array($result)) {
			...
			$r['url'] = gourl('?mid='.$r['moduleid'].'&itemid='.$r['itemid']);
			$lists[] = $r;
		}
		return $lists;
	}
	...
	function _delete($itemid) {
		$itemids = is_array($itemid) ? implode(',', $itemid) : $itemid;
		DB::query("DELETE FROM {$this->table_ban} WHERE bid IN ($itemids)");
		cache_bancomment();
	}
