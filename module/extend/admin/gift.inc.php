...
switch($action) {
	...
	case 'html':
		...
		msg('ID从'.$fid.'至'.($itemid-1).'[礼品]更新成功'.progress($sid, $fid, $tid), "?moduleid=$moduleid&file=$file&action=$action&sid=$sid&fid=$itemid&tid=$tid&num=$num&all=$all&one=$one", 0);
	break;
	case 'delete':
		$itemid or msg('请选择礼品');
		$do->delete($itemid);
	...
	case 'order_show':
		...
		if($submit) {			
			...
			if($express && $expressid) {
				...
				$auth = encrypt('gift|'.$express.'|'.$expressid.'|0|'.$user['mobile'].'|'.$itemid, DT_KEY.'EXPRESS');
			}
			$adddate = timetodate($addtime);
			$editdate = timetodate($edittime);
			$menuid = 2;
