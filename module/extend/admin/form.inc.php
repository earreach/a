...
$table = $DT_PRE.'form';
$TYPES = array('text' => '单行文本', 'textarea' => '多行文本', 'select' => '列表选择', 'checkbox' => '复选框', 'radio' => '单选框', 'date' => '日期选择', 'time' => '时间选择', 'area' => '地区选择', 'file' => '文件上传');
switch($action) {
	case 'add':
		if($submit) {
			if($do->pass($post)) {
	...
	case 'html':
		...
		msg('ID从'.$fid.'至'.($itemid-1).'[表单]更新成功'.progress($sid, $fid, $tid), "?moduleid=$moduleid&file=$file&action=$action&sid=$sid&fid=$itemid&tid=$tid&num=$num&all=$all&one=$one", 0);
	break;
	case 'delete':
		$itemid or msg('请选择表单');
		$do->delete($itemid);
	...
	case 'question':
		...
		if($job == 'add') {
			...
			if($submit) {
				...
				if(!preg_match("/^([0-9]{1,})\-([0-9]{1,})$/", $post['required'])) $post['required'] = abs(intval($post['required']));
				$post = dhtmlspecialchars($post);
				$db->query("UPDATE {$table}_question SET ".arr2sql($post, 1)." WHERE qid=$qid");
			...
			$db->query("DELETE FROM {$table}_question WHERE qid IN ($itemids)");
			$db->query("DELETE FROM {$table}_answer WHERE qid IN ($itemids)");
			dmsg('删除成功', $forward);
		} else if($job == 'update') {
			foreach($post as $k=>$v) {
				$qid = intval($k);
				...
				$name = $v['name'];
				$value = $v['value'];
				$db->query("UPDATE {$table}_question SET listorder=$listorder,name='$name',value='$value' WHERE qid=$qid");
			}
			dmsg('更新成功', '?moduleid='.$moduleid.'&file='.$file.'&action='.$action.'&fid='.$fid);
		} else if($job == 'copy') {
			if($submit) {
				if($type) {
					$ffid = intval($ffid);
	...
	case 'answer':
		...
		if($job == 'show') {
			...
			$lists = array();
			$result = $db->query("SELECT * FROM {$table}_question WHERE fid=$fid AND (type='select' OR type='checkbox' OR type='radio') ORDER BY listorder ASC,qid ASC");
			while($r = $db->fetch_array($result)) {
				$id = $r['qid'];
				$t = array();
				$t['title'] = $r['name'];
			...
			if($F['answer'] != $items && strpos($condition, ' AND') === false) $db->query("UPDATE {$table} SET answer=$items WHERE itemid=$fid");
