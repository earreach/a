...
switch($action) {
	...
	case 'group':
		if($submit) {
			...
			require_once DT_ROOT.'/module/destoon/admin/admin.class.php';
			$do = new admin;
			$do->cache_panel($_userid);
	...
	case 'html':
		...
		msg('ID从'.$fid.'至'.($itemid-1).'[单页]生成成功'.progress($sid, $fid, $tid), "?moduleid=$moduleid&file=$file&action=$action&sid=$sid&fid=$itemid&tid=$tid&num=$num&item=$item&all=$all&one=$one", 0);
	break;
	case 'delete':
		$itemid or msg('请选择单页');
		$do->delete($itemid);
	...
	break;
	default:
		$sfields = array('按条件', '标题', '链接地址', '内容', '绑定域名', '分组标识', '模板');
		$dfields = array('title','title','linkurl','content','domain','item', 'template');
