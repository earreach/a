...
switch($action) {
	...
	case 'html':
		...
		msg('ID从'.$fid.'至'.($itemid-1).'[投票]更新成功'.progress($sid, $fid, $tid), "?moduleid=$moduleid&file=$file&action=$action&sid=$sid&fid=$itemid&tid=$tid&num=$num&all=$all&one=$one", 0);
	break;
	case 'delete':
		$itemid or msg('请选择投票');
		$do->delete($itemid);
	...
	case 'record':
		...
		if($vid) $condition .= $vid == 10 ? " AND votes LIKE '%,{$vid}%'" : " AND votes LIKE '%{$vid},%'";
