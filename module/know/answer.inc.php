...
switch($action) {
	...
	default:
		...
		if($submit && $could_answer) {
			...
			$db->query("INSERT INTO {$table_answer} (qid,url,content,userid,username,passport,expert,addtime,ip,status,hidden) VALUES ('$itemid','$url','$content','$_userid','$_username','".addslashes($_passport)."','$expert','$DT_TIME','$DT_IP','$status','$hidden')");
			...
			if($status == 3) {
				...
				$forward = 'answer'.DT_EXT.'?itemid='.$itemid.'&page='.$page.'&rand='.mt_rand(10, 99).'#last';
