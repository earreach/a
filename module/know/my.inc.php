...
switch($action) {
	case 'add':
		...
		if($submit) {
			...
			$credit = 0;
			if($post['credit']) $credit += $post['credit'];
			if(isset($post['hidden'])) $credit += $MOD['credit_hidden'];
			...
			if(check_name($askto)) $p = $db->get_one("SELECT itemid,title,username,major FROM {$table_expert} WHERE username='$askto'");
			$ask = $p ? $askto : '';
			$areaid = $cityid;
			if($kw) $title = $kw;
			$item = array();
