...
switch($action) {
	case 'add':
		...
		if($submit) {
			...
			if($could_elite && isset($elite) && $post['thumbs'][0]) $credit += $MOD['credit_elite'];
			...
			if($do->pass($post)) {
				...
				if($MOD['upload_thumb'] && $MG['upload'] && !is_url($post['thumbs'][0])) dalert($L['sell_upload_image']);
				...
				if($could_elite && isset($elite) && $post['thumbs'][0]) {
					$post['level'] = 1;
