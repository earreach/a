...
class sell {
	...
	function set($post) {
		...
		$post['minamount'] = $post['price'] > 0 ? intval($post['minamount']) : dround($post['minamount']);
		$post['amount'] = $post['price'] > 0 ? intval($post['amount']) : dround($post['amount']);
		$post['maxamount'] = $post['price'] > 0 ? intval($post['maxamount']) : dround($post['maxamount']);
