...
switch($action) {
	...
	case 'batch':
		if($submit) {
			...
			$MOD['level'] = '';
			if($mid) {
				$_MOD = cache_read('module-'.$mid.'.php');
				$MOD['level'] = $_MOD['level'];
				$table = get_table($mid);
	...
	default:			
		...
		$dorder  = array('addtime DESC', 'addtime DESC', 'addtime ASC', 'edittime DESC', 'edittime ASC', 'level DESC', 'level ASC', 'itemid DESC', 'itemid ASC');
		isset($order) && isset($dorder[$order]) or $order = 0;
		(isset($fromdate) && is_time($fromdate)) or $fromdate = '';
		$fromtime = $fromdate ? datetotime($fromdate) : 0;
		(isset($todate) && is_time($todate)) or $todate = '';
