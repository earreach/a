...
if($DT_PC) {
	...
	if($cityid) {
		...
		$items = $db->count($table_group, $condition, $CFG['db_expires']*split_id($CAT['item']));
	} else {
		if($page == 1) {
			$items = $db->count($table_group, $condition, $CFG['db_expires']*split_id($CAT['item']));
			if($items != $CAT['item']) {
	...
	$tags = array();
	if($items) {
		$result = $db->query("SELECT * FROM {$table_group} WHERE {$condition} ORDER BY addtime DESC LIMIT {$offset},{$pagesize}", ($CFG['db_expires'] && $page <= $DT['cache_page']) ? 'CACHE' : '', $CFG['db_expires']);
