...
switch($action) {
	...
	case 'reply_edit':
		...
		if($submit) {
			...
			if($do->pass($post)) {		
				...
				$reason = dhtmlspecialchars($reason);
				$message = isset($message) ? 1 : 0;
				if($message) send_message($R['username'], lang($L['manage_msg_title'], array($L['my_manage_type_reply'], get_intro($R['content'], 20), $L['my_manage_type_edit'])), lang($L['manage_msg_content'], array(($DT_PC ? $MOD['linkurl'] : $MOD['mobile']).'goto'.DT_EXT.'?itemid='.$R['itemid'], nl2br($reason), $_username)));
	...
	case 'reply_delete':
		...
		foreach($itemid as $rid) {
			...
			$do->recycle($rid);
			if($message) send_message($R['username'], lang($L['manage_msg_title'], array($L['my_manage_type_reply'], get_intro($R['content'], 20), $L['my_manage_type_del'])), lang($L['manage_msg_content'], array($MOD['linkurl'].'goto'.DT_EXT.'?itemid='.$R['itemid'], nl2br($reason), $_username)));
