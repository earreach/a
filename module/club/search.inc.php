...
if($DT_PC) {
	...
	if($DT_QST) {
		...
		if($action == 'group') {
			...
			if($items) {
				$result = $db->query("SELECT * FROM {$table_group} WHERE {$condition} ORDER BY itemid DESC LIMIT {$offset},{$pagesize}", $DT['cache_search'] && $page <= $DT['cache_page'] ? 'CACHE' : '', $DT['cache_search']);
			...
			$pages = pages($items, $page, $pagesize);
			if($items) {
				$result = $db->query("SELECT * FROM {$table_reply} WHERE {$condition} ORDER BY itemid DESC LIMIT {$offset},{$pagesize}", $DT['cache_search'] && $page <= $DT['cache_page'] ? 'CACHE' : '', $DT['cache_search']);
				while($r = $db->fetch_array($result)) {
					$r['adddate'] = timetodate($r['addtime'], 5);
	...
	if($DT_QST) {
		if($action == 'group') {
			...
			if($items) {
				...
				$result = $db->query("SELECT * FROM {$table_group} WHERE {$condition} ORDER BY {$order} LIMIT {$offset},{$pagesize}", $DT['cache_search'] && $page <= $DT['cache_page'] ? 'CACHE' : '', $DT['cache_search']);
				while($r = $db->fetch_array($result)) {
					$r['adddate'] = timetodate($r['addtime'], 5);
					if($lazy && isset($r['thumb']) && $r['thumb']) $r['thumb'] = DT_STATIC.'image/lazy.gif" original="'.$r['thumb'];
					$r['alt'] = $r['title'];
			...
			if($items) {
				...
				while($r = $db->fetch_array($result)) {
					...
					$r['alt'] = $r['title'];
					if($kw) $r['title'] = str_replace($kw, '<span class="f_red">'.$kw.'</span>', $r['title']);
					$r['linkurl'] = $MOD['mobile'].'goto'.DT_EXT.'?itemid='.$r['itemid'];
					$tags[] = $r;
				}
				$db->free_result($result);
			}
