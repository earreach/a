...
switch($action) {
	case 'add':
		...
		if($M) {
			if($M['status'] == 3) message($L['my_join_repeat'], ($DT_PC ? $MOD['linkurl'] : $MOD['mobile']).rewrite('fans'.DT_EXT.'?gid='.$gid));
	...
	case 'quit':
		...
		$GRP = get_group($gid);
		($GRP && $GRP['status'] == 3) or message($L['my_not_group']);
		if(is_admin($GRP)) message($L['my_not_quit']);
		$db->query("DELETE FROM {$table_fans} WHERE gid=$gid AND username='$_username'");
		message($L['success_quit'], ($DT_PC ? $MOD['linkurl'] : $MOD['mobile']).$GRP['linkurl']);
	break;
	default:
