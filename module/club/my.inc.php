...
switch($action) {
	case 'add':
		...
		$gid or dheader('?mid='.$mid.'&action=group');
		$GRP = get_group($gid);
		($GRP && $GRP['status'] == 3) or message($L['my_not_group'], '?mid='.$mid.'&action=group');
		...
		if($ban) message($L['my_baned'], '?mid='.$mid.'&action=group');
...
if($DT_PC) {
	...
	$foot = '';
	if($action == 'add' || $action == 'edit' || $action == 'group') {
		//
	} else {
