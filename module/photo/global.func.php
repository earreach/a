...
function albumlist($item) {
	...
	if($item['album'] && $item['username']) {
		$result = DB::query("SELECT * FROM {$table} WHERE album='".addslashes($item['album'])."' AND username='$item[username]' AND status=3 AND open=3 ORDER BY addtime DESC LIMIT 1000", 'CACHE');
