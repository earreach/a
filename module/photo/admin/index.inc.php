...
if(in_array($action, array('', 'check', 'reject', 'recycle'))) {
	$sfields = array('模糊', '标题', '简介', '标签', '专辑', '会员名', '编辑', 'IP', '文件路径', '内容模板', '链接地址');
	$dfields = array('keyword', 'title', 'introduce', 'tag', 'album', 'username', 'editor', 'ip', 'filepath', 'template', 'linkurl');
	isset($fields) && isset($dfields[$fields]) or $fields = 0;
	$sorder  = array('结果排序方式', '添加时间降序', '添加时间升序', '修改时间降序', '修改时间升序', '推荐级别降序', '推荐级别升序', '浏览次数降序', '浏览次数升序', '点赞次数降序', '点赞次数升序', '反对次数降序', '反对次数升序', '收藏次数降序', '收藏次数升序', '打赏次数降序', '打赏次数升序', '打赏金额降序', '打赏金额升序', '分享次数降序', '分享次数升序', '举报次数降序', '举报次数升序', '评论数量降序', '评论数量升序', '图片数量降序', '图片数量升序', '信息ID降序', '信息ID升序');
...
switch($action) {
	...
	case 'item':
		...
		if($job == 'delete') {
			...
			dmsg('删除成功', '?moduleid='.$moduleid.'&action='.$action.'&itemid='.$itemid);
		}
		if(isset($update)) {
			$thumb = '';
			if($muti_upload) {
		...
		if(isset($update)) {
			...
			if($MOD['show_html']) tohtml('show', $module);
			dmsg($del ? '删除成功' : '保存成功', '?moduleid='.$moduleid.'&action='.$action.'&itemid='.$itemid);
		} else {
			$lists = $do->item_list("item=$itemid");
			$item = $do->get_one();
			if($items != $item['items']) $db->query("UPDATE {$table} SET items=$items WHERE itemid=$itemid");
