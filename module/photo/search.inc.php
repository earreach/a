...
if($DT_PC) {
	...
	$sfields = array($L['by_auto'], $L['by_title'], $L['by_content'], $L['by_introduce'], $L['by_album']);
	$dfields = array('keyword', 'title', 'content', 'introduce', 'album');
	...
	if($DT_QST) {
		...
		$fds = $MOD['fields'];
		$condition = '';
		$condition .= ' AND open=3 AND items>0';
