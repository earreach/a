...
class photo {
    ...
    function __construct($moduleid) {
		...
		$this->fields = array('catid','areaid','level','title','style','fee','introduce','tag','album','thumb','banner','status','hits','open','password','question','answer','username','addtime', 'editor','edittime','ip','template', 'linkurl','filepath','domain','note');
    }
		...
		if(is_array($itemid)) {
			...
			if($all) {
				...
				while($rr = DB::fetch_array($result)) {
					delete_upload($rr['thumb'], $userid, $itemid);
				}
	...
	function item_delete($itemid = 0) {
		...
		if($itemid) {
			...
			if($r) {
				delete_upload($r['thumb'], match_userid($r['thumb']), $r['item']);
