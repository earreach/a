...
class property {
	...
	function edit($post) {
		$post = $this->set($post);
	    DB::query("UPDATE {$this->table} SET ".arr2sql($post, 1)." WHERE oid=$this->oid");
		return true;
	}
	...
	function delete($itemid) {		
		...
		DB::query("DELETE FROM {$this->table} WHERE oid IN ($itemids)");
		return true;
	}

	function get_list() {
		global $pages, $page, $offset, $pagesize, $CAT, $sum;
