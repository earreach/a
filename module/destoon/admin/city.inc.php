...
$DT['city'] or msg('请先开启城市分站', '?file=setting');
...
class city {
	...
	function update($post) {
		...
		$post['name'] = trim($post['name']);
		$post['domain'] = fix_domain($post['domain']);
	    DB::query("UPDATE {$this->table} SET ".arr2sql($post, 1)." WHERE areaid=$areaid");	
		return true;
	}
	...
	function get_list($condition) {
		...
		while($r = DB::fetch_array($result)) {
			$r['linkurl'] = DT_PATH.'api/city'.DT_EXT.'?action=go&forward=&areaid='.$r['areaid'];
			$lists[] = $r;
		}
		return $lists;
	}
