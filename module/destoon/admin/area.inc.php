...
$do = new area($areaid);
$type = $do->get_type();
if($type) $menus[] = array('导入省市县', '?file='.$file.'&action=import&job=area','onclick="return confirm(\'确定导入中国省市县数据吗？ 当前数据将被覆盖 \');"');
...
switch($action) {
	...
	case 'import':
		if($type) {
			$name = $job == 'area' ? 'area2021' : 'city2021';
		} else {
			$name = 'area';
		}
...
class area {
	...
	function get_type() {
		$t = DB::get_one("SELECT * FROM {$this->table} ORDER BY areaid");
		if($t) return $t['areaid'] < 110000 ? false : true;
		return true;
	}
}
?>