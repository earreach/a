...
switch($action) {
	...
	case 'find':
		...
		dheader('?file=database&action=dict&table='.$table.'&note='.$note);
	break;
	case 'preview':
		$db->halt = 0;
		$destoon_task = '';
		...
		$tag_js = strpos($tag_safe, '$') === false ? '<script type="text/javascript" charset="'.DT_CHARSET.'" src="'.DT_PATH.'api/js'.DT_EXT.'?auth='.encrypt($tag_safe).'"></script>' : '标签包含变量，不支持JS调用';
		ob_start();
		eval($tag_code);
