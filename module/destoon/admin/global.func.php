...
function fetch_url($url, $view = 0) {
	...
	if($s) {
		...
		extract($GLOBALS, EXTR_SKIP);//spider.class get_data
...
function admin_log($level = 0) {
	global $DT, $DT_QST, $moduleid, $file, $action, $_username, $dc;
	...
	if($DT['admin_log'] == 2 || ($DT['admin_log'] == 1 && ($file == 'setting' || in_array($action, array('delete', 'edit', 'move', 'clear', 'add'))))) {
		...
		$pt = get_env('port');
		DB::query("INSERT INTO ".DT_PRE."admin_log(qstring,username,ip,port,level,logtime) VALUES('$DT_QST','$_username','".DT_IP."','$pt','$level','".DT_TIME."')");
...
function admin_check() {
	...
	if(in_array($file, array('logout', 'cloud', 'panel', 'search', 'ip', 'mobile', 'history'))) return true;//All user
	...
	if($_admin == 2) {
		...
		if(in_array($file, array('admin', 'setting', 'module', 'area', 'database', 'template', 'skin', 'file', 'log', 'update', 'group', 'fields', 'loginlog'))) return false;//Founder || Common Admin Only
	}
	return true;
}
