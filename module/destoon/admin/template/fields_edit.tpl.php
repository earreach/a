...
<td><input name="post[name]" type="text" id="name" size="20" value="<?php echo $name;?>"/> <span id="dname" class="f_red"></span>
</td>
</tr>
<tr>
<td class="tl"></td>
<td class="ts">小写字母和数字组合，不能以数字开头</td>
</tr>
<tr>
...
<td><textarea name="post[default_value]" style="width:250px;height:40px;overflow:visible;"><?php echo $default_value;?></textarea></td>
</tr>
<tr>
<td class="tl"></td>
<td class="ts">支持PHP变量，例如 $_username</td>
</tr>
<tbody id="tr_option" style="display:<?php echo in_array($html, array('select', 'radio', 'checkbox')) ? '' : 'none';?>">
<tr>
<td class="tl"><span class="f_hid">*</span> 选项值</td>
<td><textarea name="post[option_value]" style="width:250px;height:80px;overflow:visible;"><?php echo $option_value;?></textarea></td>
</tr>
<tr>
<td class="tl"></td>
<td class="ts">一行一个 选项值|选项名称* 注意*为结尾标志符</td>
</tr>
</tbody>
<tr id="tr_wh" style="display:<?php echo in_array($html, array('thumb', 'editor')) ? '' : 'none';?>">
<td class="tl"><span class="f_hid">*</span> 默认宽高</td>
<td><input name="post[width]" type="text" id="width" size="5" value="<?php echo $width;?>"/> X <input name="post[height]" type="text" id="height" size="5" value="<?php echo $height;?>"/> <?php tips('编辑器宽度填写小于100的数字，代表百分比，例如98代表98%');?></td>
</tr>
<tr>
...
对于多选框(checkbox),填非0数字表示必选个数 填长度范围表示必选个数范围<br/></td>
</tr>
<tr>
<td class="tl"><span class="f_hid">*</span> 附加内容</td>
<td><input name="post[addition]" type="text" size="60" value="<?php echo $addition;?>" id="addition"/></td>
</tr>
<tr>
<td class="tl"></td>
...
<td>
<label><input type="radio" name="post[display]" value="1"<?php echo $display ? ' checked' : '';?>/> 是</label>&nbsp;&nbsp;&nbsp;&nbsp;
<label><input type="radio" name="post[display]" value="0"<?php echo !$display ? ' checked' : '';?>/> 否</label>
</td>
</tr>
<tr>
<td class="tl"></td>
<td class="ts">如果选择否，可以手动将本字段加入到对应的模板文件里，系统将不直接显示</td>
</tr>
<tr>
<td class="tl"><span class="f_red">*</span> 前台显示</td>
<td>
<label><input type="radio" name="post[front]" value="1"<?php echo $front ? ' checked' : '';?>/> 是</label>&nbsp;&nbsp;&nbsp;&nbsp;
<label><input type="radio" name="post[front]" value="0"<?php echo !$front ? ' checked' : '';?>/> 否</label>
</td>
</tr>
<tr>
<td class="tl"></td>
<td class="ts">如果选择是，则会在前台显示，会员可以修改</td>
</tr>
<tr id="tr_sc" style="display:<?php echo in_array($html, array('select', 'radio', 'checkbox')) ? '' : 'none';?>;">
<td class="tl"><span class="f_hid">*</span> 参与搜索</td>
<td>
...
<td><input name="post[note]" type="text" id="note" size="60" value="<?php echo $note;?>"/></td>
</tr>
<tr>
<td class="tl"></td>
...
<div class="sbt"><input type="submit" name="submit" value="<?php echo $action == 'edit' ? '修 改' : '添 加';?>" class="btn-g"/>&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" value="<?php echo $action == 'edit' ? '返 回' : '取 消';?>" class="btn" onclick="Go('?moduleid=<?php echo $moduleid;?>&file=<?php echo $file;?>&tb=<?php echo $tb;?>');"/></div>
</form>
...
function dlimit(id) {
	if(id == 'notnull') {
		...
		Dd('input_limit').value = '[A-Za-z]{1,}';
	} else if(id == 'nl') {
		Dd('input_limit').value = '[A-Za-z0-9]{1,}';
	} else if(id == 'email') {
...
Menuon(<?php echo $action=='add' ? 0 : 1;?>);
</script>
