...
global $db, $DT_PRE, $DT_TIME, $MODULE, $_userid;
// 启动 session，这样才能读到 quote_userid
...
// 当前登录的“报价用户”身份（验证码登录优先，其次是正常会员登录）
...
// 构造查询条件：
// 1）带 token 的访问（邮件回执等），按 itemid + receipt_token 校验；
// 2）不带 token 的访问（从“我的报价”列表进入），按 itemid + userid 校验，必须本人登录。
if($token) {
    // 旧模式：itemid + token（保持原有行为）
    if(!$itemid) {
        message('链接参数错误');
    }
    $where = "q.itemid={$itemid} AND q.receipt_token='{$token}'";
} else {
    // 新模式：仅带 itemid，从“我的报价”列表进入，仅允许本人查看
    if(!$itemid) {
        message('链接参数错误');
    }
    if(!$quote_userid) {
        message('请先通过邮箱验证码登录后再查看此报价');
    }
    $where = "q.itemid={$itemid} AND q.userid={$quote_userid}";
}
...
if($just_changed_confirm) {
    ...
    // 类型：4 = 用户接受报价通知，5 = 用户暂不接受通知
    $mail_type = ($confirm_action == 'confirm') ? 4 : 5;
