...
//    die();
    ...
    // 统一 JSON 错误输出（报价提交全部走 Ajax）
    if(!function_exists('quote_json_error')) {
        function quote_json_error($msg) {
    ...
    // -------- 2）基础校验 --------
    if($devices_json == '') {
        quote_json_error('设备故障数据丢失，请返回重新选择');
    }

    $devices = json_decode($devices_json, true);
    if(!$devices || !is_array($devices)) {
        quote_json_error('设备故障数据解析失败，请返回重新选择');
    }
    ...
    if(!$company_id) {
        quote_json_error('请选择维修门店');
    }

    if($first_name == '' || $last_name == '') {
        quote_json_error('请填写姓和名');
    }

    if($email == '') {
        quote_json_error('请填写邮箱地址');
    }

    // 邮箱验证码必填
    if($email_code == '') {
        quote_json_error('请填写邮箱验证码');
    }

    // 预约日期和时间必填
    if($appoint_date == '' || $appoint_hour === '') {
        quote_json_error('请选择预约日期和时间');
    }
    ...
    // -------- 3）图片验证码 --------
    if($captcha == '') {
        quote_json_error('请填写图片验证码');
    }
    if(function_exists('captcha')) {
        // 你那边已经确认：captcha() 返回空字符串表示正确
        if(captcha($captcha) !== '') {
            quote_json_error('图片验证码不正确');
        }
    }
...
//    die();
    ...
    if(!$ok) {
        quote_json_error('保存报价失败：'.$db->errno.' '.$db->error);
    }
    ...
    $company_email = '';
    if($company_id) {
        $c = $db->get_one("SELECT company,email FROM {$table_company} WHERE userid={$company_id}");
        if($c) {
