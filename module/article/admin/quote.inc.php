...
if($action == 'list' || $action == '') {
    ...
    // 排序参数：sort 为排序字段，order 为方向
    $sort  = isset($sort) ? trim($sort) : '';
    $order = isset($order) ? strtoupper(trim($order)) : '';
    ...
    switch($sort) {
        case 'itemid':
            // 按 ID 排序
            $order_sql = "ORDER BY q.itemid {$order}";
            break;
        case 'user_confirm_time':
            // 按用户确认时间排，附加管理员编辑时间 & ID 兜底
            $order_sql = "ORDER BY q.user_confirm_time {$order}, q.edittime DESC, q.itemid DESC";
            break;
        case 'appoint_time':
            // 按预约时间排，附加 ID 兜底
            $order_sql = "ORDER BY q.appoint_time {$order}, q.itemid DESC";
            break;
        case 'edittime':
            // 按管理员操作时间排
            $order_sql = "ORDER BY q.edittime {$order}, q.itemid DESC";
            break;
        case 'addtime':
            // 按用户提交时间排序，附带 ID 兜底
            $order_sql = "ORDER BY q.addtime {$order}, q.itemid DESC";
            break;

        default:
            // 默认：先按用户确认时间，再按管理员操作时间，最后按 ID
            $sort      = 'user_confirm_time';
            $order     = 'DESC';
            $order_sql = "ORDER BY q.user_confirm_time DESC, q.edittime DESC, q.itemid DESC";
            break;
    }
...
// ====== 查看 + 编辑 ======
    ...
    if($submit) {
        ...
        // 邮件标题 & 三块正文（本次发送用，可在审核页编辑）
        $mail_subject          = isset($_POST['mail_subject']) ? trim($_POST['mail_subject']) : '';
        $mail_body_top         = isset($_POST['mail_body_top']) ? trim($_POST['mail_body_top']) : '';
        $mail_body_middle_pref = isset($_POST['mail_body_middle_prefix']) ? trim($_POST['mail_body_middle_prefix']) : '';
        $mail_body_bottom      = isset($_POST['mail_body_bottom']) ? trim($_POST['mail_body_bottom']) : '';

        // 边界保护
        if($status_new < 0 || $status_new > 2) $status_new = 0;
        if($final_amount < 0) $final_amount = 0;
        if($discount_amount < 0) $discount_amount = 0;
        if($repair_amount < 0) $repair_amount = 0;

        // 取旧记录
        $r = $db->get_one("SELECT * FROM {$table} WHERE itemid={$itemid}");
        $r or msg('记录不存在');

        // === 新一轮报价：根据旧状态决定是否重置用户确认 & 维修状态 ===
        $old_user_confirm_status = isset($r['user_confirm_status']) ? intval($r['user_confirm_status']) : 0;
        $old_user_confirm_time   = isset($r['user_confirm_time']) ? intval($r['user_confirm_time']) : 0;
        $old_repair_status       = isset($r['repair_status']) ? intval($r['repair_status']) : 0;

        // 默认保持原来的用户确认状态
        $new_user_confirm_status = $old_user_confirm_status;
        $new_user_confirm_time   = $old_user_confirm_time;

        // 当本次审核设为“通过”(status=1)，且之前用户已经表态过(!=0)，视为开启新一轮报价
        if($status_new == 1 && $old_user_confirm_status != 0) {
            // 让用户在回执页重新选择：接受 / 暂不接受
            $new_user_confirm_status = 0;
//            $new_user_confirm_time   = 0;

            // 如果管理员没有主动改维修状态，且旧状态是“已取消(4)”，则拉回“未设置(0)”
            if($repair_status == $old_repair_status && $old_repair_status == 4) {
                $repair_status = 0;
            }
        }
        ...
        $db->query("UPDATE {$table} 
            SET final_amount='".number_format($final_amount, 2, '.', '')."',
                ...
                user_confirm_status={$new_user_confirm_status},
                user_confirm_time={$new_user_confirm_time},
                status={$status_new},
        ...
        if($need_send) {
            ...
            // 收件人信息
            $to_email = $r['email'];
            $to_name  = $r['first_name'].$r['last_name'];

            // 邮件类型：1=首次报价，2=调整后报价（按发送次数判断）
            $send_times = 0;
            if(isset($r['receipt_send_count'])) {
                $send_times = intval($r['receipt_send_count']);
            } else if(isset($r['notify_sent']) && $r['notify_sent']) {
                $send_times = 1;
            }
            $mail_type = $send_times > 0 ? 2 : 1;
            ...
            // 入队前做简单转义
            $subject_sql  = addslashes($mail_subject);
            $top_sql      = addslashes($mail_body_top);
            $middle_sql   = addslashes($mail_body_middle_pref);
            $bottom_sql   = addslashes($mail_body_bottom);
            $to_name_sql  = addslashes($to_name);
            $to_email_sql = addslashes($to_email);

            $amount = $final_amount;

            // 避免同一条报价重复插入未发送队列：有就更新，没有再插入
            $queue = $db->get_one("SELECT id FROM {$queue_table} WHERE itemid={$itemid} AND status=0");
            if($queue) {
                $db->query("UPDATE {$queue_table}
                            SET type={$mail_type},
                                to_email='{$to_email_sql}',
                                to_name='{$to_name_sql}',
                                subject='{$subject_sql}',
                                body_top='{$top_sql}',
                                body_middle_prefix='{$middle_sql}',
                                body_bottom='{$bottom_sql}',
                ...
                $db->query("INSERT INTO {$queue_table}
                            ...
                            ({$itemid}, {$mail_type}, '{$to_email_sql}', '{$to_name_sql}', '{$subject_sql}',
                             ...
                             '".number_format($amount, 2, '.', '')."', 0, 0, {$DT_TIME}, 0, '')");
            }
        }
