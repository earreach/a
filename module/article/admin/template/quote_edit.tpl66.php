...
<form method="post" action="?moduleid=<?php echo $moduleid;?>&file=<?php echo $file;?>&action=edit&itemid=<?php echo $item['itemid'];?>">
    ...
    <!-- ★ 在设备 & 故障明细下面展示金额汇总 -->
    <?php
    // 先把金额安全地转成 float，避免空字符串导致 number_format 报 Notice
    $__total_fault_amount = isset($item['total_fault_amount']) ? floatval($item['total_fault_amount']) : 0;
    $__discount_amount    = isset($item['discount_amount']) ? floatval($item['discount_amount']) : 0;
    $__final_amount       = isset($item['final_amount']) ? floatval($item['final_amount']) : 0;

    // 如果 total_fault_amount 还是 0，但 $devices 里已经有 price，就用设备里的 price 再算一遍
    if($__total_fault_amount <= 0 && !empty($devices)) {
        $__total_fault_amount = 0;
        foreach($devices as $d) {
            if(empty($d['faults']) || !is_array($d['faults'])) continue;
            foreach($d['faults'] as $f) {
                if(isset($f['price']) && $f['price'] !== '') {
                    $__total_fault_amount += floatval($f['price']);
                }
            }
        }
        // 如果数据库里的 final_amount 也是 0，就按公式重新算一遍
        if($__final_amount <= 0) {
            $__final_amount = max(0, $__total_fault_amount - $__discount_amount);
        }
    }

//var_dump($__total_fault_amount);
//    die();

    ?>
    <!-- ★ 在设备 & 故障明细下面展示金额汇总 -->
    <?php
    // 安全转换金额，避免空字符串导致 number_format 报 Notice
    $__total_fault_amount = isset($item['total_fault_amount']) ? floatval($item['total_fault_amount']) : 0;
    $__discount_amount    = isset($item['discount_amount']) ? floatval($item['discount_amount']) : 0;
    $__final_amount       = isset($item['final_amount']) ? floatval($item['final_amount']) : 0;

    // 如果数据库里是 0，但设备明细中已经有 price，则用设备里的 price 再算一遍，兼容老数据
    if($__total_fault_amount <= 0 && !empty($devices)) {
        $__total_fault_amount = 0;
        foreach($devices as $d) {
            if(empty($d['faults']) || !is_array($d['faults'])) continue;
    ...
    <div style="margin-top:10px;padding:8px 10px;border:1px solid #f0f0f0;background:#fcfcfc;">
        ...
        <span id="total_fault_amount" data-value="<?php echo $__total_fault_amount;?>">
            <?php echo number_format($__total_fault_amount, 2);?> 円
        </span>
        &nbsp;&nbsp;
        <strong>优惠码抵扣金额：</strong>
        <span id="coupon_amount">
            <?php echo number_format($__discount_amount, 2);?> 円
        </span>
        &nbsp;&nbsp;
        <strong>优惠金额：</strong>
        <input type="text" id="admin_discount" name="post[discount_amount]" size="10"
               value="<?php echo $item['discount_amount'] !== '' ? $item['discount_amount'] : '0';?>"
               oninput="this.value=this.value.replace(/[^0-9.]/g,'');updateFinalAmount();"/>
        円
        &nbsp;&nbsp;
        <strong>最终报价金额：</strong>
        <span id="final_amount" class="f_red">
            <?php echo number_format($__final_amount, 2);?> 円
        </span>
        <span class="f_gray">（最终报价 = 故障原价合计 - 优惠金额）</span>
    </div>
    <script type="text/javascript">
        function updateFinalAmount() {
            ...
            if(!input) return;
            var d = parseFloat(input.value || '0');
            if(isNaN(d)) d = 0;

            var finalVal = total - d;
