...
<form method="post" action="?moduleid=<?php echo $moduleid;?>&file=<?php echo $file;?>&action=edit&itemid=<?php echo $item['itemid'];?>">
    ...
    <script type="text/javascript">
        ...
        function updateFinalAmount() {
            ...
            // 3）邮件正文中间的金额：整数 + 千分位
            var mailAmountSpan = document.getElementById('mail_amount_display');
            if(mailAmountSpan) {
                mailAmountSpan.innerHTML = formatIntAmount(finalVal);
            }
        }

        // 页面加载完成后，先跑一次计算，保证默认值正确
...
//    die();
    ...
    // === 报价邮件默认文案（三块） ===
    // 顶部：问候 + 解释本次邮件背景，不带具体金额
    $mail_body_top_lines   = array();
    $mail_body_top_lines[] = '您好，'.$customer_name.'：';

    if($send_times > 0) {
        if($last_confirm == 2) {
            // 上一次明确“暂不接受”
            $mail_body_top_lines[] = '根据您上次“暂不接受”的反馈，我们对报价做了调整。';
        } else {
            // 只是长时间没回复或其他情况
            $mail_body_top_lines[] = '我们对您的报价信息进行了更新。';
        }
    } else {
        // 第一次发送
        $mail_body_top_lines[] = '感谢您提交维修报价请求，我们已为您的设备完成评估。';
    }
    $mail_body_top_default = implode("\n\n", $mail_body_top_lines);

    // 中间报价行前缀（金额数字放在中间那块，只读）
    if($send_times > 0) {
        $mail_body_middle_prefix_default = '当前新的报价金额为：';
    } else {
        $mail_body_middle_prefix_default = '当前报价金额为：';
    }

    // 底部：预约时间 + 引导查看详情，不带具体金额数字
    $mail_body_bottom_lines   = array();
    $mail_body_bottom_lines[] = '预约到店时间：'.$appoint_str;
    $mail_body_bottom_lines[] = '详细报价内容请通过下方链接查看。';
    $mail_body_bottom_default = implode("\n\n", $mail_body_bottom_lines);
    ?>
        ...
        <tr>
            ...
            <td class="tr">
                ...
                <div style="margin-bottom:8px;">
                    <strong>上半段正文：</strong><br/>
                    <textarea name="mail_body_top"
                              id="mail_body_top"
                              style="width:400px;height:80px;"><?php echo htmlspecialchars($mail_body_top_default, ENT_QUOTES, 'UTF-8');?></textarea>
                </div>

                <!-- 中间报价行：前缀 + 自动金额 + 单位 -->
                <div style="margin-bottom:8px;">
                    <strong>报价金额行：</strong><br/>
                    <input type="text"
                           name="mail_body_middle_prefix"
                           id="mail_body_middle_prefix"
                           style="width:260px;"
                           value="<?php echo htmlspecialchars($mail_body_middle_prefix_default, ENT_QUOTES, 'UTF-8');?>"/>
                    <span id="mail_amount_display"
                          class="f_red"
                          style="font-weight:bold;">
                        <?php echo number_format($final_amount, 0);?>
                    </span>
                    円
                    <br/>
                    <span class="f_gray">
                        左侧为“当前报价金额为：”这类文案，可按需修改；中间金额为最终报价金额，只读，随上方优惠金额自动计算。
                    </span>
                </div>

                <!-- 下半段正文 -->
                <div style="margin-bottom:8px;">
                    <strong>下半段正文：</strong><br/>
                    <textarea name="mail_body_bottom"
                              id="mail_body_bottom"
                              style="width:400px;height:80px;"><?php echo htmlspecialchars($mail_body_bottom_default, ENT_QUOTES, 'UTF-8');?></textarea>
                </div>

                <span class="f_gray">
                    邮件发送时，系统会按「上半段 → 报价金额行 → 下半段」的顺序拼接正文，
                    并在末尾自动附加管理员备注（如有）和报价回执链接。
                </span>
    ...
    <div class="sbt">
        <!-- 确认（通过并发送邮件） -->
        <input type="submit" value="确认(发送邮件)" class="btn"
