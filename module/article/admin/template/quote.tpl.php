...
<?php
// 当前筛选条件拼成基础 URL（用于表头排序链接）
$baseurl = '?moduleid='.$moduleid.'&file='.$file;
$baseurl .= '&status='.$status;
$baseurl .= '&company_id='.$company_id;
$baseurl .= '&keyword='.urlencode($keyword);
$baseurl .= '&fromdate='.urlencode($fromdate);
$baseurl .= '&todate='.urlencode($todate);
$baseurl .= '&appointdate='.urlencode($appointdate);
$baseurl .= '&repair_status='.$repair_status;
$baseurl .= '&user_confirm_status='.$user_confirm_status;

// 当前排序字段和方向（给模板用，避免未定义）
$current_sort  = isset($sort)  ? $sort  : 'user_confirm_time';
$current_order = isset($order) ? $order : 'DESC';
?>
...
<table cellpadding="2" cellspacing="1" class="tb">
    <tr>
        <th width="40">
            <?php
            $next_order = ($current_sort == 'itemid' && $current_order == 'DESC') ? 'ASC' : 'DESC';
            ?>
            <a href="<?php echo $baseurl.'&sort=itemid&order='.$next_order;?>">
                ID<?php
                if($current_sort == 'itemid') {
                    // 当前按 ID 排：显示具体方向
                    echo $current_order == 'DESC' ? '▼' : '▲';
                } else {
                    // 默认状态：显示一个中性箭头，表示可排序
                    echo '▼';
                }
        ...
        <th width="110">报价邮件</th>
        <th width="80">状态</th>
        <th width="80">用户确认状态</th>
        <th width="80">
            <?php
            $next_order = ($current_sort == 'user_confirm_time' && $current_order == 'DESC') ? 'ASC' : 'DESC';
            ?>
            <a href="<?php echo $baseurl.'&sort=user_confirm_time&order='.$next_order;?>">
                用户确认时间<?php
                if($current_sort == 'user_confirm_time') {
                    echo $current_order == 'DESC' ? '▼' : '▲';
                } else {
                    echo '▼';
                }
        ...
        <th width="120">
            <?php
            $next_order = ($current_sort == 'appoint_time' && $current_order == 'DESC') ? 'ASC' : 'DESC';
            ?>
            <a href="<?php echo $baseurl.'&sort=appoint_time&order='.$next_order;?>">
                预约时间<?php
                if($current_sort == 'appoint_time') {
                    echo $current_order == 'DESC' ? '▼' : '▲';
                } else {
                    echo '▼';
                }
    ...
    <?php if(!empty($lists)) { ?>
        <?php foreach($lists as $k => $v) { ?>
            <tr align="center">
                ...
                <td>
                    <?php if(!empty($v['receipt_send_count'])) { ?>
                        <?php echo intval($v['receipt_send_count']);?>次<br/>
                        <?php if(!empty($v['last_receipt_send_time'])) { ?>
                            <?php echo timetodate($v['last_receipt_send_time'], 5); ?>
                        <?php } ?>
                    <?php } else { ?>
                        <span class="f_gray">未发送</span>
                    <?php } ?>
                </td>
