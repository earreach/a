...
login();
($MG['biz'] && $MG['homepage'] && $MG['news_limit'] > -1) or dheader(($DT_PC ? $MOD['linkurl'] : $MOD['mobile']).'account'.DT_EXT.'?action=group&itemid=1');
	...
	case 'edit':
		...
		if($submit) {
			if($do->pass($post)) {
				...
				set_cookie('dmsg', $post['status'] == 2 ? $L['op_edit_check'] : $L['op_edit_success']);
				dalert('', '', 'parent.window.location="'.($post['status'] == 2 ? '?status=2' : $forward).'"');
			} else {
				dalert($do->errmsg);
			}
		} else {
