...
login();
($MG['biz'] && $MG['homepage'] && $MG['honor_limit'] > -1) or dheader(($DT_PC ? $MOD['linkurl'] : $MOD['mobile']).'account'.DT_EXT.'?action=group&itemid=1');
	...
	case 'edit':
		...
		if($submit) {
			if($do->pass($post)) {
				...
				$do->edit($post);
				if($post['status'] < 3 && $item['status'] > 2) history($moduleid, 'honor-'.$itemid, 'set', $item);
