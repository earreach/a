...
class message {
    ...
    function __construct() {
		global $_userid, $_username, $_passport;
		$this->userid = $_userid;
	...
	function send($message) {
		...
		if(isset($message['save'])) {
			...
			if(check_name($message['touser']) && blacked($message['touser'])) return $this->_($L['message_msg_black']);
			$feedback = isset($message['feedback']) ? 1 : 0;
			foreach(explode(' ', $message['touser']) as $touser) {
				if(!check_name($touser) || $this->username == $touser) continue;
				$tpassport = get_user($touser, 'username', 'passport');
				if(!$tpassport) return false;
				if(blacked($touser, $this->username)) continue;
				if(isset($message['copy'])) DB::query("INSERT INTO ".DT_PRE."message (title,typeid,content,fromuser,fpassport,touser,tpassport,addtime,ip,feedback,status) VALUES ('$message[title]','$message[typeid]','$message[content]','$this->username','$this->passport','$touser','$tpassport','".DT_TIME."','".DT_IP."','$feedback','2')");
				DB::query("UPDATE ".DT_PRE."member SET message=message+1 WHERE username='$touser'");
				DB::query("INSERT INTO ".DT_PRE."message (title,typeid,content,fromuser,fpassport,touser,tpassport,addtime,ip,feedback,status) VALUES ('$message[title]','$message[typeid]','$message[content]','$this->username','$this->passport','$touser','$tpassport','".DT_TIME."','".DT_IP."','$feedback','3')");				
				$this->itemid = DB::insert_id();
				if($DT['push_appkey'] && $DT['push_secret'] && $DT['push_ok']) send_push($touser, $message['title'], '', $MODULE[2]['mobile'].'message'.DT_EXT.'?action=show&itemid='.$this->itemid);
			}
		}
	...
	function edit($message) {
		...
		$message['title'] = dhtmlspecialchars(trim($message['title']));
		$message['content'] = dsafe(addslashes(save_remote(save_local(stripslashes($message['content'])))));
		delete_diff($message['content'], $r['content'], $r['itemid']);
	...
	function get_list($condition, $order = 'itemid DESC') {
		...
		$messages = array();
		$result = DB::query("SELECT itemid,title,style,typeid,fromuser,fpassport,touser,tpassport,addtime,ip,isread,issend,feedback,status,groupids,mid,tid FROM ".DT_PRE."message WHERE {$condition} ORDER BY {$order} LIMIT {$offset},{$pagesize}");
		while($r = DB::fetch_array($result)) {
			$r['adddate'] = timetodate($r['addtime'], 5);
			$r['alt'] = $r['title'];
			$r['dtitle'] = dsubstr($r['title'], 55, '...');
			if($kw) $r['title'] = str_replace($kw, '<span class="f_red">'.$kw.'</span>', $r['title']);
			#$r['user'] = $r['status'] > 2 ? ($r['fromuser'] ? ($r['fpassport'] ? $r['fpassport'] : $r['fromuser']) : $L['message_from_system']) : ($r['tpassport'] ? $r['tpassport'] : $r['touser']);
			if($r['fromuser']) {
				$r['user'] =  $r['status'] > 2 ? ($r['fpassport'] ? $r['fpassport'] : $r['fromuser']) : ($r['tpassport'] ? $r['tpassport'] : $r['touser']);
				$r['userurl'] = userurl($r['status'] > 2 ? $r['fromuser'] : $r['touser'], 'file=space');
				$r['username'] = $r['status'] > 2 ? $r['fromuser'] : $r['touser'];
			} else {
	...
	function delete($recycle = 0, $black = '') {
		...
		while($r = DB::fetch_array($result)) {
			if(defined('DT_ADMIN')) {
				...
				if($black && check_name($r['fromuser'])) black_add($r['fromuser'], $black);
			}
		}
	}
	...
	function _clear($message) {
		...
		if(isset($message['username']) && check_name($message['username'])) $condition .= " AND touser='$message[username]'";
