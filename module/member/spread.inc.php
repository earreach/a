...
$MG['spread'] or dheader(($DT_PC ? $MOD['linkurl'] : $MOD['mobile']).'account'.DT_EXT.'?action=group&itemid=1');
...
if($action == 'add') {
	...
	if($word && $mid > 3 && isset($MODULE[$mid]) && !$MODULE[$mid]['islink']) {
		...
		$next_time = datetotime($next_year.'-'.$next_month.'-1');
		...
		if($submit) {
			...
			$totime = datetotime($to_year.'-'.$to_month.'-1');
			...
			$db->query("INSERT INTO {$DT_PRE}spread (mid,tid,word,price,currency,company,username,addtime,fromtime,totime,status) VALUES ('$mid','$buy_tid','$word','$amount','$currency','$_company','$_username','$DT_TIME','$next_time','$totime','$status')");
	...
	while($r = $db->fetch_array($result)) {
		...
		$r['url'] = gourl('?mid='.$r['mid'].'&itemid='.$r['tid']);
