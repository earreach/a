...
class honor {
    ...
    function __construct() {
		...
		$this->fields = array('title','style','content', 'authority','thumb','status','username','addtime','editor','edittime','ip','fromtime','totime','note');
    }
	...
	function add($post) {
		...
		if($post['username'] && $MOD['credit_add_honor']) {
			credit_add($post['username'], $MOD['credit_add_honor']);
			credit_record($post['username'], $MOD['credit_add_honor'], 'system', $L['honor_reward_reason'], 'ID:'.$this->itemid);
		}
	...
	function delete($itemid, $all = true) {
		...
		if(is_array($itemid)) {
			...
			if($r['username'] && $MOD['credit_del_honor']) {
				credit_add($r['username'], -$MOD['credit_del_honor']);
				credit_record($r['username'], -$MOD['credit_del_honor'], 'system', $L['honor_punish_reason'], 'ID:'.$this->itemid);
			}
