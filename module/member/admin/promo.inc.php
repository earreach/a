...
switch($action) {
	case 'clear':
		...
		$db->query("DELETE FROM {$DT_PRE}finance_promo WHERE totime<$time AND totime>0");
		dmsg('清理成功', $forward);
	break;
	case 'clear_coupon':
		$time = $DT_TODAY - 30*86400;
		$db->query("DELETE FROM {$DT_PRE}finance_coupon WHERE totime<$time AND totime>0 AND oid=0");
		dmsg('清理成功', $forward);
	break;
	case 'add':
		if($submit) {
	...
	case 'send':
		...
		if($submit) {
			...
			if($r['totime'] && $r['totime'] < $DT_TIME) msg('优惠活动已结束');
			$title = addslashes($r['title']);
			$db->query("INSERT INTO {$DT_PRE}finance_coupon (title,username,seller,addtime,fromtime,totime,price,cost,pid,mid,catid,itemids,editor,edittime,note) VALUES ('$title','$username','$r[username]','$DT_TIME','$r[fromtime]','$r[totime]','$r[price]','$r[cost]','$itemid','$r[mid]','$r[catid]','$r[itemids]','$_username','$DT_TIME','$note')");
	...
	case 'coupon':
		...
		$sorder  = array('排序方式', '领取时间降序', '领取时间升序', '优惠额度降序', '优惠额度升序', '最低消费降序', '最低消费升序', '开始时间降序', '开始时间升序', '结束时间降序', '结束时间升序',  '更新时间降序', '更新时间升序');
		$dorder  = array('itemid DESC', 'addtime DESC', 'addtime ASC', 'price DESC', 'price ASC', 'cost DESC', 'cost ASC', 'fromtime DESC', 'fromtime ASC', 'totime DESC', 'totime ASC', 'edittime DESC', 'edittime ASC');
		...
		isset($datetype) or $datetype = 'addtime';
		...
		$lists = array();
		$result = $db->query("SELECT * FROM {$DT_PRE}finance_coupon WHERE {$condition} ORDER BY {$dorder[$order]} LIMIT {$offset},{$pagesize}");
	...
	default:
		...
		isset($fields) && isset($dfields[$fields]) or $fields = 0;
		$sorder  = array('排序方式', '添加时间降序', '添加时间升序', '优惠额度降序', '优惠额度升序', '最低消费降序', '最低消费升序', '数量限制降序', '数量限制升序', '领券人数降序', '领券人数升序', '开始时间降序', '开始时间升序', '结束时间降序', '结束时间升序', '更新时间降序', '更新时间升序');
		$dorder  = array('itemid DESC', 'addtime DESC', 'addtime ASC', 'price DESC', 'price ASC', 'cost DESC', 'cost ASC', 'amount DESC', 'amount ASC', 'number DESC', 'number ASC', 'fromtime DESC', 'fromtime ASC', 'totime DESC', 'totime ASC', 'edittime DESC', 'edittime ASC');
		isset($order) && isset($dorder[$order]) or $order = 0;
