...
switch($action) {
	...
	case 'delete':
		...
		$db->query("DELETE FROM {$DT_PRE}mail WHERE itemid IN ($itemids)");
		dmsg('删除成功', $forward);
	break;
	case 'list_delete':
		$itemid or msg('未选择记录');
		$itemids = is_array($itemid) ? implode(',', $itemid) : $itemid;
		$db->query("DELETE FROM {$DT_PRE}mail_list WHERE itemid IN ($itemids)");
		dmsg('删除成功', $forward);
	break;
	case 'send':
		$itemid or msg();
	...
	case 'list':
		...
		$condition = '1';
		if($keyword) $condition .= match_kw('email', $keyword);
	...
	if($username) $condition .= " AND username='$username'";
		...
		while($r = $db->fetch_array($result)) {
			...
			$r['type'] = '<select onchange="if(this.value){Dq(\'typeid\',this.value);}">';
			foreach($typeids as $t) {
				$r['type'] .= '<option value="'.$t.'"'.($t == $typeid ? ' selected' : '').'>'.$TYPE[$t]['typename'].'</option>'; 
			}
			$r['type'] .= '</select>';
			$lists[] = $r;
		}
	...
	break;
	default:
		$sorder  = array('结果排序方式', '添加时间降序', '添加时间升序', '更新时间降序', '更新时间升序', '发送时间降序', '发送时间升序');
		$dorder  = array('itemid DESC', 'addtime DESC', 'addtime ASC', 'edittime DESC', 'edittime ASC', 'sendtime DESC', 'sendtime ASC');
		isset($order) && isset($dorder[$order]) or $order = 0;

		$typeid = isset($typeid) ? ($typeid === '' ? -1 : intval($typeid)) : -1;
		isset($datetype) && in_array($datetype, array('addtime', 'edittime', 'sendtime')) or $datetype = 'addtime';
		(isset($fromdate) && is_time($fromdate)) or $fromdate = '';
		$fromtime = $fromdate ? datetotime($fromdate) : 0;
		(isset($todate) && is_time($todate)) or $todate = '';
		$totime = $todate ? datetotime($todate) : 0;
		...
		$pages = pages($items, $page, $pagesize);	
		$lists = array();
		$result = $db->query("SELECT * FROM {$DT_PRE}mail WHERE {$condition} ORDER BY {$dorder[$order]} LIMIT {$offset},{$pagesize}");
		while($r = $db->fetch_array($result)) {
			$r['addtime'] = timetodate($r['addtime'], 5);
			$r['edittime'] = timetodate($r['edittime'], 5);
