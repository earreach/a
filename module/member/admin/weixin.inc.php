...
if($openid) {
	...
	$menus = array (
		...
		array('公众平台', gourl('https://mp.weixin.qq.com/'), 'target="_blank"'),
	);
}
switch($action) {
	case 'unbind':
		...
		$result = $db->query("SELECT * FROM {$DT_PRE}weixin_user WHERE itemid IN ($itemids)");
		while($user = $db->fetch_array($result)) {
			$db->query("UPDATE {$DT_PRE}oauth SET username='' WHERE openid='$user[openid]' AND site='weixin'");
		}
	...
	case 'remove':
		...
		$result = $db->query("SELECT * FROM {$DT_PRE}weixin_user WHERE itemid IN ($itemids)");
		while($user = $db->fetch_array($result)) {
			$db->query("DELETE FROM {$DT_PRE}oauth WHERE openid='$user[openid]' AND site='weixin'");
		}
		$db->query("DELETE FROM {$DT_PRE}weixin_user WHERE itemid IN ($itemids)");
		dmsg('删除成功', $forward);
	break;
	case 'edit':
		$itemid or msg('请选择记录');
		...
		if($submit) {
			...
			$db->query("UPDATE {$DT_PRE}oauth SET username='$name' WHERE openid='$user[openid]' AND site='weixin'");
	...
	case 'clear':
		...
		$db->query("DELETE FROM {$DT_PRE}weixin_chat WHERE addtime<$time");
		dmsg('清理成功', $forward);
	break;
	case 'delete':
		$itemid or msg('请选择记录');
	...
	case 'load':
		...
		$json = '{chat_msg:['.$json.'],chat_new:"'.$j.'",chat_last:"'.$chatlast.'",chat_bug:"'.$debug.'"}';
		exit($json);
	break;
	case 'user':
		$SEX = array('未知', '男', '女');
		...
		$lists = array();
		$result = $db->query("SELECT * FROM {$DT_PRE}weixin_user WHERE {$condition} ORDER BY {$dorder[$order]} LIMIT {$offset},{$pagesize}");
		while($r = $db->fetch_array($result)) {
	...
	case 'auto':
		if($job == 'delete') {
			...
			$itemids = is_array($itemid) ? implode(',', $itemid) : $itemid;
			$db->query("DELETE FROM {$DT_PRE}weixin_auto WHERE itemid IN ($itemids)");
			dmsg('删除成功', $forward);
		} else if($job == 'update') {
			foreach($post as $k=>$v) {
				$k = intval($k);
...
function weixin_msg($type, $content, $misc) {
	...
	switch($type) {
		...
		break;
		case 'voice':
			return '<a href="javascript:;" onclick="Dwidget(\'api/weixin/media'.DT_EXT.'?action='.$type.'&mediaid='.$misc['MediaId'].'\', \'播放语音 - QuickTime Player\', 300, 16, \'no\');"><img src="api/weixin/image/media_voice.gif" align="absmiddle"/></a>&nbsp;&nbsp;<a href="api/weixin/down'.DT_EXT.'?mediaid='.$misc['MediaId'].'" class="t">下载</a>';
		break;
		case 'video':
			return '<a href="javascript:;" onclick="Dwidget(\'api/weixin/media'.DT_EXT.'?action='.$type.'&mediaid='.$misc['MediaId'].'\', \'播放视频 - QuickTime Player\', 300, 400, \'no\');"><img src="api/weixin/image/media_video.gif" align="absmiddle"/></a>&nbsp;&nbsp;<a href="api/weixin/down'.DT_EXT.'?mediaid='.$misc['MediaId'].'" class="t">下载</a>';
		break;
		case 'location':
			return '<img src="api/weixin/image/media_map_marker.gif" align="absmiddle"/> <a href="javascript:;" onclick="Dwidget(\'api/weixin/media'.DT_EXT.'?action='.$type.'&latitude='.$misc['Location_X'].'&longitude='.$misc['Location_Y'].'&zoom='.$misc['Scale'].'\', \'查看地图 - '.$content.'\', 450, 400, \'no\');" class="t">'.$content.'</a>';
		break;
		case 'link':
		...
		case 'reply':
			if($misc['type'] == 'image') {
				return weixin_msg($misc['type'], 'api/weixin/down'.DT_EXT.'?mediaid='.$content, '');
		...
		break;
		case 'LOCATION':
			return '<a href="javascript:;" onclick="Dwidget(\'api/weixin/media'.DT_EXT.'?action=location&latitude='.$misc['Latitude'].'&longitude='.$misc['Longitude'].'\', \'查看地图\', 450, 400, \'no\');"><img src="api/weixin/image/media_map.gif" align="absmiddle"/></a>';
		break;
		default:
			return $content;
		break;
