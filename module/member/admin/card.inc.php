...
switch($action) {
	case 'add':
		if($submit) {
			...
			if(!is_time($totime)) msg('过期时间格式错误');
			$totime = datetotime($totime);
			...
			$totime = (timetodate($DT_TIME, 'Y') + 3).timetodate($DT_TIME, '-m-d').' 23:59:59';
	...
	default:
		...
		if($status == 1) $condition .= " AND updatetime=0 AND totime>$DT_TIME";
		if($status == 2) $condition .= " AND updatetime>0";
		if($status == 3) $condition .= " AND updatetime=0 AND totime<$DT_TIME";
		if($page > 1 && $sum) {
			$items = $sum;
		} else {
		...
		while($r = $db->fetch_array($result)) {
			...
			$r['updatedate'] = $r['updatetime'] ? timetodate($r['updatetime'], 5) : 'N/A';
			$lists[] = $r;
		}
