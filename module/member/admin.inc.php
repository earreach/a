	...
	if($admin_user) {
		...
		if($_userid && $_username == $_USER[1]) {
			...
			if($__userid && !is_founder($__userid)) {
				$USER = $db->get_one("SELECT username,passport,company,truename,mobile,password,groupid,gradeid,fans,follows,moments,validate,email,message,chat,sound,online,sms,credit,money,loginip,admin,role,aid,edittime FROM {$DT_PRE}member WHERE userid=$__userid");
				if($USER) {
