...
class address {
    ...
    function __construct() {
		...
		$this->fields = array('typeid','typename','areaid','address','postcode','truename','telephone','mobile','username','addtime','editor','edittime','listorder','note');
    }
	...
	function pass($post) {
		...
		if(!$post['postcode'] && $DT['postcode']) return $this->_($L['pass_postcode']);
		if(!$post['truename']) return $this->_($L['pass_truename']);
		if(!is_mobile($post['mobile'])) return $this->_($L['pass_mobile']);
		return true;
	}

	function set($post) {
		global $_username, $_cname, $L;
		...
		if(!isset($L['address_type'][$post['typeid']])) $post['typeid'] = 0;
		if($post['typeid']) {
			if($post['typename']) {
				if($post['typename'] != $L['address_type'][$post['typeid']]) $post['typeid'] = 0;
			} else {
				$post['typename'] = $L['address_type'][$post['typeid']];
			}
		}
		if($this->itemid) {
			//
