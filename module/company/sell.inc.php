...
(isset($orderby) && in_array($orderby, array('dmessages', 'dorders', 'dprice', 'aprice'))) or $orderby = '';
if($itemid) {
	...
	$could_purchase = ($price > 0 && $minamount > 0 && $amount > 0 && $unit && $username && $username != $_username && !$expired) ? 1 : 0;
	...
	$typeid = $mycatid;
	$inquiry_url = $MODULE[4]['linkurl'].'home'.DT_EXT.'?action=message&job=inquiry&&itemid='.$itemid.'&template='.$template.'&skin='.$skin.'&title='.rawurlencode($title).'&username='.$username.'&sign='.crypt_sign($itemid.$template.$skin.$title.$username);
	$order_url = $MODULE[4]['linkurl'].'home'.DT_EXT.'?action=message&job=order&&itemid='.$itemid.'&template='.$template.'&skin='.$skin.'&title='.rawurlencode($title).'&username='.$username.'&sign='.crypt_sign($itemid.$template.$skin.$title.$username);
	$update = '';
