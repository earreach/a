...
(isset($orderby) && in_array($orderby, array('danswer', 'dhits', 'dcomments'))) or $orderby = '';
...
if($itemid) {
	...
	if($item['hidden']) dheader($MENU[$menuid]['linkurl']);
	unset($item['template']);
	...
	$url = "file=$file";
	$condition = "username='$username' AND status=3 AND hidden=0";
	if($kw) {
