...
if(in_array($action, array('', 'check', 'reject', 'recycle'))) {
	$sfields = array('模糊', '标题', '简介', '标签', '专辑', '视频地址', '会员名', '编辑', 'IP', '文件路径', '内容模板', '链接地址');
	$dfields = array('keyword', 'title', 'introduce', 'tag', 'album', 'video', 'username', 'editor', 'ip', 'filepath', 'template', 'linkurl');
	isset($fields) && isset($dfields[$fields]) or $fields = 0;
	$sorder  = array('结果排序方式', '添加时间降序', '添加时间升序', '修改时间降序', '修改时间升序', '推荐级别降序', '推荐级别升序', '浏览次数降序', '浏览次数升序', '点赞次数降序', '点赞次数升序', '反对次数降序', '反对次数升序', '收藏次数降序', '收藏次数升序', '打赏次数降序', '打赏次数升序', '打赏金额降序', '打赏金额升序', '分享次数降序', '分享次数升序', '举报次数降序', '举报次数升序', '评论数量降序', '评论数量升序', '视频时长降序', '视频时长升序', '信息ID降序', '信息ID升序');
	$dorder  = array($MOD['order'], 'addtime DESC', 'addtime ASC', 'edittime DESC', 'edittime ASC', 'level DESC', 'level ASC', 'hits DESC', 'hits ASC', 'likes DESC', 'likes ASC', 'hates DESC', 'hates ASC', 'favorites DESC', 'favorites ASC', 'awards DESC', 'awards ASC', 'award DESC', 'award ASC', 'shares DESC', 'shares ASC', 'reports DESC', 'reports ASC', 'comments DESC', 'comments ASC', 'duration DESC', 'duration ASC', 'itemid DESC', 'itemid ASC');
	...
	$tags = isset($tags) ? intval($tags) : 0;
	$albums = isset($albums) ? intval($albums) : 0;
	$poster = isset($poster) ? intval($poster) : 0;
	...
	if($tags) $condition .= " AND tag<>''";
	if($albums) $condition .= " AND album<>''";
	if($poster) $condition .= " AND poster<>''";
