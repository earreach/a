...
function msg($msg = errmsg, $forward = 'goback', $time = '1') {
	...
	include DT_ROOT.'/module/destoon/admin/template/msg.tpl.php';
    exit;
}

function dialog($dcontent) {
	global $CFG, $DT_MBS;
	include DT_ROOT.'/module/destoon/admin/template/dialog.tpl.php';
    exit;
}

function tpl($file = 'index', $mod = 'destoon') {
	global $CFG, $DT;
	return DT_ROOT.'/module/'.$mod.'/admin/template/'.$file.'.tpl.php';
}
...
function progress($sid, $fid, $tid) {
	...
	return '<table cellpadding="0" cellspacing="0" width="100%" style="margin:10px 0 0 0;"><tr><td><div class="progress"><div style="width:'.$p.';">&nbsp;</div></div></td><td style="color:#666666;font-size:10px;width:40px;text-align:right;">'.$p.'</td></tr></table>';
}

function show_menu($menus = array()) {
    global $moduleid, $mid, $module, $file, $action, $_admin;
    $menu = '';
    ...
    if($moduleid==21&&$file=='setting')	$menu.= '<td id="Tab111" class="'.$class.'"><a href="?moduleid='.$moduleid.'&file=shuxing">'.定义属性mdag.'</a></td>';

    include DT_ROOT.'/module/destoon/admin/template/menu.tpl.php';
}
...
function fetch_url($url, $view = 0) {
	$tmp = parse_url($url);
	$domain = $tmp['host'];
	$result = DB::query("SELECT * FROM ".DT_PRE."fetch WHERE domain='$domain' ORDER BY itemid DESC");
	$post = $list = $arr = $s = array();	
	$html = '';
	while($r = DB::fetch_array($result)) {
		if(!$r['setting']) continue;
		if($r['mark']) {
			if(strpos($url, $r['mark']) !== false) {
				$s = $r;
				break;
			}
		} else {
	...
	if(!$s && isset($arr[0])) $s = $arr[0];
	if($s) {
		$html = dcurl($url);
		$s['setting'] = unserialize($s['setting']);
		extract($GLOBALS, EXTR_SKIP);
		/*spider.class get_data()*/
		foreach($s['setting'] as $v) {
			$k = $v['fk'];
	...
	return $view ? array($post, $list, $html, $s) : $post;
}

function admin_log($level = 0) {
	global $DT, $DT_QST, $moduleid, $file, $action, $job, $itemid, $_username, $dc;
	if($level) $DT['admin_log'] = 2;
	...
	if($DT['admin_log'] == 2 || ($DT['admin_log'] == 1 && ($file == 'setting' || in_array($action, array('delete', 'edit', 'move', 'clear', 'add'))))) {
		if(strpos($DT_QST, 'file=admin&action=log') !== false) return false;
		$qstring = cutstr($DT_QST, '', '&forward');
		if($moduleid > 1 && strpos($qstring, 'moduleid=') === false) $qstring .= '&moduleid='.$moduleid;
		if($file && strpos($qstring, 'file=') === false) $qstring .= '&file='.$file;
		if($action && strpos($qstring, 'action=') === false) $qstring .= '&action='.$action;
		if($job && strpos($qstring, 'job=') === false) $qstring .= '&job='.$job;
		if($itemid && strpos($qstring, 'itemid=') === false) $qstring .= '&itemid='.(is_array($itemid) ? implode(',', $itemid) : $itemid);
		$md5string = md5($qstring);
		if($dc->get('logstring-'.$_username.DT_IP) == $md5string)  return false;
		$pt = get_env('port');
		DB::query("INSERT INTO ".DT_PRE."admin_log(qstring,username,ip,port,level,logtime) VALUES('$qstring','$_username','".DT_IP."','$pt','$level','".DT_TIME."')");
		$dc->set('logstring-'.$_username.DT_IP, $md5string);
	}
}
...
function admin_check() {
	global $CFG, $_admin, $_userid, $moduleid, $file, $action, $catid, $_catids, $_childs, $_self;
	if(!check_name($file)) return false;
	if(is_founder($_userid)) return true;
	if(in_array($file, array('logout', 'cloud', 'panel', 'search', 'ip', 'mobile', 'history'))) return true;
	if($moduleid == 1 && $file == 'index') return true;
	if($_admin == 2) {
		$R = cache_read('admin-right-'.$_userid.'.php');
		if(!$R) return false;
		if(!isset($R[$moduleid])) return false;
		if(!$R[$moduleid]) return true;
		if(!isset($R[$moduleid][$file])) return false;
		if(!$R[$moduleid][$file]) return true;
		if(isset($R[$moduleid][$file]['self']) && $R[$moduleid][$file]['self']) $_self = 1;
		...
		if(in_array($file, array('admin', 'setting', 'module', 'area', 'database', 'template', 'skin', 'file', 'log', 'update', 'group', 'fields', 'loginlog'))) return false;
	}
	return true;
}
...
function split_content($moduleid, $part) {
	...
	DB::query("CREATE TABLE IF NOT EXISTS `{$table}` (`{$fd}` bigint(20) unsigned NOT NULL default '0',`content` mediumtext NOT NULL,PRIMARY KEY  (`{$fd}`))".$type." COMMENT='".$MODULE[$moduleid]['name']."内容_".$part."'");
}

function split_sell($part) {
	global $CFG, $MODULE;
	$sql = file_get(DT_ROOT.'/file/setting/table_split_sell.php');
	$sql or dalert('请检查文件file/setting/table_split_sell.php是否存在');
...
function seo_title($title, $show = '') {
	...
	if(is_array($show)) {
		foreach($show as $v) {
			if(isset($SEO[$v])) echo '<a href="javascript:;" onclick="_into(\''.$title.'\', \'{'.$SEO[$v].'}\');" title="{'.$SEO[$v].'}">{'.$SEO[$v].'}</a>&nbsp;&nbsp;';
		}
	} else {
		foreach($SEO as $k=>$v) {
...
function install_home($moduleid, $file, $dir, $extend = 0) {
	$HM = cache_read('home.php');
	if(isset($HM['menu'][$dir]) && !is_numeric($HM['menu'][$dir])) return false;
	$content = "<?php\n";
	if($extend == 1) $content .= "define('DT_REWRITE', true);\n";
	$content .= "\$file = '".$dir."';\n";
	$content .= "\$moduleid = 4;\n";
	$content .= "require '../../common.inc.php';\n";
	$content .= "require DT_ROOT.'/module/'.\$module.'/index.inc.php';\n";
	$content .= '?>';
	file_put(DT_ROOT.'/company/'.$dir.'/'.$file.'.php', $content);
}
...
function fields_exists($fields, $table) {
	$result = DB::query("SHOW COLUMNS FROM `{$table}`");
	while($r = DB::fetch_array($result)) {
		if($r['Field'] == $fields) return 1;
	}
	return 0;
}
?>