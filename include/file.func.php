...
function file_ico($filename) {
	$ext = strpos($filename, '.') === false ? $filename : file_ext($filename);
	return is_file(DT_ROOT.'/file/ext/'.$ext.'.gif') ? $ext : 'oth';
}
...
function file_get($filename) {
	...
	if($rec) return $rec;
	if(substr($filename, 0, 8) == 'https://') return file_get('http://'.substr($filename, 8));
	return '';
}
...
function file_copy($from, $to) {
	...
	if(strpos($from, '://') !== false && strpos($from, DT_PATH) !== false && strpos(DT_PATH, '://') !== false) $from = str_replace(DT_PATH, DT_ROOT.'/', $from);
	if(@copy($from, $to)) {
		if(DT_CHMOD) @chmod($to, DT_CHMOD);
		return true;
	} else {
		if(substr($from, 0, 8) == 'https://') return file_copy('http://'.substr($from, 8), $to);
		return false;
	}
}
...
function html_del($filename) {
	file_del($filename);
	file_del(str_replace(DT_ROOT, DT_ROOT.'/mobile', $filename));
}
...
function dir_delete($dir) {
	...
	$dirs = array(DT_ROOT.'/admin/', DT_ROOT.'/api/', DT_CACHE.'/', DT_ROOT.'/file/', DT_ROOT.'/cache/', DT_ROOT.'/include/', DT_ROOT.'/lang/', DT_ROOT.'/member/', DT_ROOT.'/module/', DT_ROOT.'/skin/', DT_ROOT.'/template/', DT_ROOT.'/mobile/');
