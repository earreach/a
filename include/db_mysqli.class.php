...
class db_mysqli {
	...
	function fetch_array($query, $result_type = DB_ASSOC) {
	...
	function halt($message = '', $sql = '')	{
		...
		if($this->halt) message('MySQL Query:'.str_replace(DT_PRE, '[pre]', $sql).' <br/> MySQL Error:'.str_replace(DT_PRE, '[pre]', $this->error()).' MySQL Errno:'.$this->errno().' <br/>Message:'.$message);
	}
	...
	function _query($sql, $ttl) {
		...
		if(!is_array($this->result)) {
			...
			while($r = $this->fetch_array($result, DB_ASSOC)) {
				$tmp[] = $r; 
			}
			$this->result = $tmp;
