...
function property_search_arr($catid) {
	$catid = intval($catid);
	$lists = array();
	$result = DB::query("SELECT * FROM ".DT_PRE."category_option WHERE catid=$catid AND type>1 AND search>0 ORDER BY listorder ASC,oid ASC", 'CACHE');
	while($r = DB::fetch_array($result)) {
		$r['options'] = explode('|', str_replace('(*)', '', $r['value']));
		$r['select'] = '';
		$lists[] = $r;
...
function property_search_sql() {
	global $PP, $_GET;
	$condition = '';
	foreach($PP as $k=>$v) {
		if($v['search'] && isset($_GET['pp'.$v['oid']])) {
			$vv = $_GET['pp'.$v['oid']];
			$PP[$k]['select'] = strip_kw($vv);
			if(in_array($vv, $v['options'])) {
				$condition .= match_kw('pptword', 'O'.$v['oid'].':'.$vv.';');
			}
		}
	}
	return $condition;
