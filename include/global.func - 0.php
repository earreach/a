...
function encrypt($txt, $key = '', $ttl = 0) {
	...
	return str_replace(array('=', '+', '/', '0x', '0X'), array('-E-', '-P-', '-S-', '-Z-', '-X-'), mycrypt($str, $key, 0, $ttl));
}
...
function send_sms($mobile, $message, $word = 0, $cron = 0) {
	global $DT, $_username;
	if(!$DT['sms'] || !$DT['sms_sign'] || !DT_CLOUD_UID || !DT_CLOUD_KEY || !is_mobile($mobile) || strlen($message) < 5) return 0;
	if(strpos($message, substr($DT['sms_sign'], 0, 3)) === false) $message = $DT['sms_sign'].$message;
	$word or $word = word_count($message);
	$sms_message = $message;
	$data = 'sms_uid='.DT_CLOUD_UID.'&sms_key='.md5(DT_CLOUD_KEY.'|'.DT_TIME.'|'.$mobile.'|'.md5($sms_message)).'&sms_charset='.DT_CHARSET.'&sms_mobile='.$mobile.'&sms_time='.DT_TIME.'&sms_cron='.$cron.'&sms_message='.rawurlencode($sms_message).'&sms_url='.rawurlencode(DT_PATH);
	...
	if($code && strpos($code, 'destoon_sms_code=') !== false) {
		...
		if(strpos($code, $DT['sms_ok']) !== false) $status = 3;
	} else {
