...
if(isset($username)) {
	...
	if($mid == 1) {
		...
		if($itemid) {
			if(in_array($tb, array('news', 'honor', 'page'))) {
				$r = $db->get_one("SELECT linkurl FROM {$DT_PRE}{$tb} WHERE itemid=$itemid");			
		...
		if($itemid) {
			if(in_array($tb, array('announce', 'webpage', 'link', 'gift', 'vote', 'poll', 'form'))) {
				...
				$r = $db->get_one("SELECT item_mid,item_id,status,fid FROM {$DT_PRE}comment WHERE itemid=$itemid");
				if($r && $r['status'] == 3) {
					$page = $r['fid'] ? ceil($r['fid']/$EXT['comment_pagesize']) : 1;
					$url = $EXT['comment_url'].rewrite('index.php?mid='.$r['item_mid'].'&itemid='.$r['item_id'].($page > 1 ? '&page='.$page : '')).'#H'.$itemid;
				}
			}
		}
	} else if($mid == 4) {
	...
	(check_referer() && is_url($url)) or $url = DT_PATH;
