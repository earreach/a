...
($WX['appid'] && $WX['appsecret'] && $WX['apptoken']) or message('Missing appid or secret');
...
function weixin_user($openid) {
	...
	if($r) return $r;
	DB::query("INSERT INTO ".DT_PRE."weixin_user (username,openid,addtime,logintime,loginip,logintimes) VALUES ('$_username','$openid','".DT_TIME."','".DT_TIME."','".DT_IP."','1')");
	$itemid = DB::insert_id();
	return DB::get_one("SELECT * FROM ".DT_PRE."weixin_user WHERE itemid=$itemid");
}
...
function weixin_oauth($openid) {
	$W = weixin_user($openid);
	$O = DB::get_one("SELECT * FROM ".DT_PRE."oauth WHERE openid='$openid' AND site='weixin'");
	if($O) {
		...
		DB::query("INSERT INTO ".DT_PRE."oauth (username,nickname,avatar,site,openid,addtime,logintime,loginip,logintimes) VALUES ('$W[username]','".addslashes($W['nickname'])."','".addslashes($W['headimgurl'])."','weixin','$openid','".DT_TIME."','".DT_TIME."','".DT_IP."','1')");
	}
	DB::query("INSERT INTO ".DT_PRE."oauth_login (username,site,loginip,loginport,logintime,agent) VALUES ('".$W['username']."','weixin','".DT_IP."','".get_env('port')."','".DT_TIME."','".addslashes(dhtmlspecialchars(strip_sql(strip_tags(DT_UA))))."')");
}
