		...
		if($MsgType == 'event') {//事件
			...
			switch($Event) {
				case 'CLICK'://点击菜单拉取消息时的事件推送
					switch($EventKey) {
						case 'V_member'://绑定会员
							...
							$tags['url'] = DT_MOB.'api/weixin'.DT_EXT.'?action=member&auth='.encrypt("$FromUserName", DT_KEY.'WXID');
							$misc[] = $tags;						
				...
				case 'subscribe'://订阅
					...
					$sql = "subscribe=1,addtime=$stime,edittime=$DT_TIME,visittime=$DT_TIME,logintime=$DT_TIME,loginip='$DT_IP'";
					...
					if(strpos($post['content'], 'qrscene_') !== false) {
						$sid = substr($post['content'], 8);
						$B = is_md5($sid) ? $db->get_one("SELECT * FROM {$DT_PRE}weixin_bind WHERE sid='$sid'") : array();
						if($B) {
					...
					weixin_oauth($FromUserName);
					//回复欢迎消息
					$wx->response($FromUserName, $ToUserName, 'text', $WX['welcome']."\n\n已是会员？请先绑定:\n".DT_MOB."api/weixin".DT_EXT."?action=connect&url=bind\n\n不是会员？请先注册:\n".DT_MOB."api/weixin".DT_EXT."?action=connect");
				break;
				case 'unsubscribe'://取消订阅
					$post['content'] = '';
					$db->query("UPDATE {$DT_PRE}weixin_user SET subscribe=0,edittime=$DT_TIME WHERE openid='$FromUserName'");
				break;
				case 'SCAN'://扫描二维码[已关注]
					$post['content'] = $EventKey;
					$post['misc']['Ticket'] = "$x->Ticket";
					...
					if(is_md5($EventKey)) {
						...
						if($B) {//待绑定
							...
							$S = $db->get_one("SELECT * FROM {$DT_PRE}weixin_scan WHERE sid='$sid'");
							if($S) {							
								$db->query("UPDATE {$DT_PRE}weixin_scan SET openid='$FromUserName' WHERE sid='$sid'");
								$U = $db->get_one("SELECT * FROM {$DT_PRE}weixin_user WHERE openid='$FromUserName'");
								//回复欢迎消息
								if($U && $U['username']) {
									$T['username'] = $U['username'];
									$wx->response($FromUserName, $ToUserName, 'text', '会员['.$U['username'].']微信扫码登录成功');
								} else {
									$wx->response($FromUserName, $ToUserName, 'text', "扫码成功\n\n已是会员？请先绑定:\n".DT_MOB."api/weixin".DT_EXT."?action=connect&url=bind\n\n不是会员？请先注册:\n".DT_MOB."api/weixin".DT_EXT."?action=connect");
								}
							}
						}
						weixin_oauth($FromUserName);
			...
			switch($MsgType) {
				case 'text'://文本消息
					...
					if($Content == '签到') {
						...
						$code = random(6, '0-9');
						$db->query("REPLACE INTO {$DT_PRE}weixin_code (openid,username,code,addtime) VALUES ('$FromUserName','$name','$code','$DT_TIME')");
						$wx->response($FromUserName, $ToUserName, 'text', '验证码：'.$code.'，有效期10分钟，切勿透露给他人');
					} else {
						//自动回复
