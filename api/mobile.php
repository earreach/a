<?php
$moduleid = 3;
require '../common.inc.php';
require DT_ROOT.'/module/'.$module.'/common.inc.php';
$MOD['mobile_enable'] or dheader(DT_PATH);
require DT_ROOT.'/include/post.func.php';
$ck = get_cookie('mobile');
$url = DT_MOB;
if(isset($uri) && is_url($uri)) {
	if($action == 'device') $url = DT_PATH;
	if(is_uri($uri)) $url = $uri;
}
if($action == 'pc') {
	set_cookie('mobile', 'pc');
	dheader(DT_PATH);
} else if($action == 'screen') {
	$DT_MOB == 'ios' or exit;
	if($ck != 'screen') set_cookie('mobile', 'screen');
} else {
	if(strpos($DT_URL, 'action=sync&auth=') !== false && strpos($DT_URL, 'goto=') !== false) {
		if($DT_MOB == 'ios') {
			isset($auth) or $auth = '';
			$auth = decrypt($auth, DT_KEY.'SCREEN');
			if($auth) {
				$arr = explode('|', $auth);
				if(check_name($arr[0]) && $_username != $arr[0] && $DT_IP == $arr[1] && $DT_TIME - $arr[2] < 600) {
					include load('member.lang');
					$MOD = cache_read('module-2.php');
					include DT_ROOT.'/module/member/member.class.php';
					$do = new member;
					$user = $do->login($arr[0], '', 0, 'ios-sync');
				}
			}
			$tmp = explode('goto=', $DT_URL);
			$goto = $tmp[1];
			if(preg_match("/^[a-z0-9_\.\?\&\=\-]{5,}$/", $goto)) {
				if(strpos($goto, '://') === false) $goto = $MODULE[2]['linkurl'].$goto;
				$url = $goto;
			}
		}
		dheader($url);
	}
	if($ck != 'pc') {
		if(is_mob()) dheader($url);
		set_cookie('mobile', 'pc');
	}
	$ios_app = $EXT['mobile_ios'] ? DT_PATH.'api/qrcode'.DT_EXT.'?auth='.urlencode(DT_PATH.'api/app'.DT_EXT) : '';
	$android_app = $EXT['mobile_adr'] ? DT_PATH.'api/qrcode'.DT_EXT.'?auth='.urlencode(DT_PATH.'api/app'.DT_EXT) : '';
	$wxmini = is_file(DT_ROOT.'/api/weixin/image/wxmini.png') ?  DT_PATH.'api/weixin/image/wxmini.png' : '';
	$destoon_task = rand_task();
	$head_title = $L['mobile_title'];
	$head_mobile = $url;
	include template('mobile', $module);
}
?>