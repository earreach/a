...
if($mid > 3 && $itemid > 0) {
	...
	if($table) {
		...
		$t = $db->get_one("SELECT itemid FROM {$DT_PRE}favorite WHERE userid=$_userid AND mid=$mid AND tid=$itemid", 'CACHE');
		if($t) {
			$dc->remove("SELECT itemid FROM {$DT_PRE}favorite WHERE userid=$_userid AND mid=$mid AND tid=$itemid");
