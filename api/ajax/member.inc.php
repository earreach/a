...
switch($job) {
	...
	break;
	case 'truename':
		if(strlen($value) < 2 || !is_clean($value)) exit($L['member_truename_null']);
	break;
	case 'qq':
		if(!is_qq($value)) exit($L['member_qq_null']);
	break;
	case 'wx':
		if(!is_wx($value)) exit($L['member_wx_null']);
	break;
	case 'get_company':
		$user = $do->get_one($value);
		if($user) {
			echo '<a href="'.$user['linkurl'].'" target="_blank" class="t">'.$user['company'].'</a>'.( $user['vip'] ? ' <img src="'.DT_SKIN.'vip.gif" align="absmiddle"/> <img src="'.DT_SKIN.'vip_'.$user['vip'].'.gif" align="absmiddle"/>' : '');
		} else {
			echo '1';
		}
		exit;
