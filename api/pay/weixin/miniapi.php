...
$charge_title = '';
$auth = isset($auth) ? decrypt($auth, DT_KEY.'MINIPAY') : '';
...
$PAY[$bank]['appid'] = $WX['wxmini_appid'];
#$PAY[$bank]['keycode'] = $WX['wxmini_appsecret'];
...
<script type="text/javascript" src="//res.wx.qq.com/open/js/jweixin-1.6.0.js?v=<?php echo DT_DEBUG ? DT_TIME : DT_REFRESH;?>"></script>
<script type="text/javascript">
var interval = window.setInterval(
	function() {
		$.get('qrcode.php?action=ajax&itemid=<?php echo $itemid;?>', function(data) {
			if(data == 'ok') {
				clearInterval(interval);
				window.location.href = '<?php echo $MODULE[2]['mobile'];?>charge.php';
			}
		});
	}, 
2000);
function CallPay() {
	wx.miniProgram.navigateTo({url:'/pages/wxpay/wxpay?appId=<?php echo $arr['appId'];?>&timeStamp=<?php echo $arr['timeStamp'];?>&nonceStr=<?php echo $arr['nonceStr'];?>&prepay_id=<?php echo $x->prepay_id;?>&signType=<?php echo $arr['signType'];?>&paySign=<?php echo $arr['paySign'];?>'});
}
CallPay();
</script>
</body>
