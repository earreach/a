...
if($r['reason']) {
	...
	if($arr[0] == 'trade' || $arr[0] == 'trades') {
		foreach(explode(',', $arr[1]) as $id) {
			...
			if($O && $O['buyer'] == $r['username'] && $O['status'] == 1) {
				$mallid = $O['mallid'];
				...
				if($m['money'] >= $money) {
					...
					$db->query("UPDATE {$table} SET status=2,updatetime=$DT_TIME WHERE pid=$itemid");
					$db->query("INSERT INTO {$table}_log (oid,addtime,title,note) VALUES ('$itemid','$DT_TIME','$L[log_pay]','')");
					$touser = $O['seller'];
		...
		$O = $db->get_one("SELECT * FROM {$table} WHERE itemid=$itemid");
		if($O && $O['buyer'] == $_username && $O['status'] == 6) {
			$m = $db->get_one("SELECT money FROM {$DT_PRE}member WHERE username='$r[username]'");
			$money = $O['amount'];
			if($m['money'] >= $money) {
				money_add($_username, -$money);
