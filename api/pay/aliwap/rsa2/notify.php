...
if(strlen($PAY[$bank]['public']) < 10) exit('fail');
...
//log_write(array('post'=>$_POST, 'res'=>$result), 'alin', 1);
/* 实际验证过程建议商户添加以下校验。
...
if($result) {//验证成功
	...
	//支付金额
	$total_fee = dround($_POST['total_amount']);
    ...
    if($_POST['trade_status'] == 'TRADE_FINISHED') {
		...
		if($r) {
			if($r['status'] == 0) {
				...
				$editor = 'N'.$bank;
				if(dround($total_fee) == dround($charge_money)) {
