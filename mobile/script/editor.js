...
$.fn.extend({
    ...
    DEditor: function (config) {
        ...
        if(_this.config.textareaid) {
            ...
            $(_this).on('input', function () {
				var eid = $(_this).attr('id');
				if(eid == 'editor') {//Fix Muti Editor
					$('#'+_this.config.textareaid).val(_this.getval());
				} else {
					$('#'+eid.substring(7)).val(_this.getval());
				}
            });
        }
        ...
        if(!/firefox/.test(UA) && this.config.breaks) {
	...
	action: function() {
        var _this = this;
        $('.ui-editor-toolbar input').on('click', function(e) {
			document.execCommand($(this).attr('editor-action'), false, null);
	...
	insert: function (htm) {
		...
		if(eid == 'editor') {//Fix Muti Editor
			...
			$('#'+eid.substring(7)).val(this.getval());
		}
    },
    getval: function () {
        return $(this).html();
