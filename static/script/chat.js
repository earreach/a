...
function chat_load(d){
	$.post('chat'+DTExt, 'action=load&chatlast='+chat_last+'&chatid='+chat_id+'&first='+(d ? 1 : 0), function(data) {
		if(data) {
			...
			if(msglen && d) {$('#chat').append('<div class="chat-more"><a href="im'+DTExt+'?action=view&chatid='+chat_id+'" target="_blank"><span>'+L['chat_record']+'</span></a></div>');}
			for(var i=0;i<msglen;i++){
				...
				if(chat_msg[i].self == 1) {
					...
					msghtm += '<td width="60" valign="top" align="center"><a href="'+chat_home1+'" target="_blank"><img src="'+chat_head1+'" class="chat_head"/></a></td>';
					msghtm += '<td width="10"></td>';
				} else {
					msghtm += '<td width="10"></td>';
					msghtm += '<td width="60" valign="top" align="center"><a href="'+chat_home0+'" target="_blank"><img src="'+chat_head0+'" class="chat_head"/></a></td>';
			...
			$('#chat_name').html(chat_nick ? chat_nick+'@' : '');
		}
	});
}
function chat_log(){
