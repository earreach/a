...
var color_id = 1; var color_bk = color_htm = color_oid = '';
...
function color_select(color) {color=color.toUpperCase();if(color.length>0&&!color.match(/^#[A-F0-9]{6}$/)){return;}color_hide(); if(color_oid){Dd(color_oid).value = color;} Dd('color_input_'+color_id).value = color; Dd('color_img_'+color_id).style.backgroundColor = color;}
