...
function ca_setup(year,month){
	...
	if(ca_tm){
		calendar += '<tr style="height:48px;text-align:center;"><td style="border-top:#E0E0E0 1px solid;"><img src="'+DTPath+'static/image/time.png" width="16" height="16"/></td><td colspan="5" style="border-top:#E0E0E0 1px solid;">';
		...
		calendar += '</select></td><td style="border-top:#E0E0E0 1px solid;cursor:pointer;" title="OK" onclick="ca_close();"><img src="'+DTPath+'static/image/enter.png" width="16" height="16"/></td></tr>';
	}
	calendar += '</table>';
...
function ca_time() {
	Dd(ca_id).value=(Dd(ca_id).value ? Dd(ca_id).value.substring(0,8+ca_sep.length*2) : t_year+ca_sep+ca_padzero(t_month)+ca_sep+ca_padzero(t_day))+' '+Dd('ca_hour').value+':'+Dd('ca_minute').value+':'+Dd('ca_second').value;
}
