...
function uperr($errmsg, $errjs = '') {
	...
	if($from == 'editor' && (DT_EDITOR == 'ueditor' || DT_EDITOR == 'umeditor')) {
...
if($from == 'thumb' || $from == 'album' || $from == 'photo' || $from == 'file') {
	...
	if(DT_EDITOR == 'fckditor') $errjs .= 'window.parent.GetE("frmUpload").reset();';
}
if($do->save()) {
	...
	if($DT['convert']) {
		...
		if(isset($CV[$do->ext])) {
			$ext = $CV[$do->ext];
			$db->query("INSERT INTO {$DT_PRE}upload_convert (fileurl,filesize,fileext,toext,addtime,edittime,username) VALUES ('$saveto','$do->file_size','$do->ext','$ext','$DT_TIME','$DT_TIME','$_username')");
			$savecp = substr($do->saveto, 0, -strlen($do->ext)).$ext;
			$savedm = DT_ROOT.'/static/image/convert.'.$ext;
			if(is_file($savedm)) {
				file_copy($savedm, DT_ROOT.'/'.$savecp);
				touch(DT_ROOT.'/'.$savecp, $DT_TIME);
			}
			if($DT['convert'] == 1) $DT['ftp_save'] = 1;
		}
	}
	if($DT['ftp_remote'] && $DT['remote_url']) {
		...
		if($ftp->connected) {
			$remote = cutstr($do->saveto, 'file/upload/', '');
			if($ftp->dftp_put($do->saveto, $remote)) {
				...
				if(strpos($do->saveto, '.thumb.') !== false) {
					...
					$ftp->dftp_put($local, cutstr($local, 'file/upload/', ''));
	...
	if($savecp && file_ext($savecp) == 'mp4') $saveto = substr($saveto, 0, -strlen($do->ext)).'mp4';
	$fid = isset($fid) ? $fid : '';
	...
	if($from == 'thumb') {
		...
		$js .= 'try{'.$pr.'("p'.$fid.'").src="'.$saveto.'";}catch(e){}';
